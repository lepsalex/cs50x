...
// set request line constants for array
#define METHOD          0
...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // Strip out return characters (CRLF)
            char cleanLine[needle - haystack + 1];
            strncpy(cleanLine, haystack, needle - haystack);
            cleanLine[needle-haystack] = '\0';
            
            // Extract each parameter (token)
            char* params[3] = {NULL, NULL, NULL};            
            params[METHOD] = strtok(cleanLine, " ");
            int n = 0;
            while (params[n] != NULL && n < 2)
            {
                n++;
            ...
            // Validate method
            if (strcmp(params[METHOD], "GET") != 0 || params[METHOD] == NULL)
            {
                error(405);
                continue;
            }
            ...
            if ((params[REQUEST_TARGET] == NULL) || (strchr(params[REQUEST_TARGET], '\"') != NULL))
            {
                error(400);
                continue;
            }
            else if (params[REQUEST_TARGET][0] != '/')
            {   
                error(501);
                continue;
            }
            ...
            if (strcmp(params[HTTP_VERSION], "HTTP/1.1") != 0 || params[HTTP_VERSION] == NULL)
            {
                error(505);
                continue;
            }
            ...
            // Start by getting position of '?' and end of request
            char *questionPtr = strchrnul(params[REQUEST_TARGET], '?');
            char *endRT = strchr(params[REQUEST_TARGET], '\0');
            
            // Declare int to get lenght of query the calculate based on position of pointers above
            int qryLength = endRT - questionPtr;
            
            if (qryLength < 1)
                qryLength = 1;
                       
            char query[qryLength];
            
            // Point to location of query start
            char* qryPtr;
            if (strncmp(questionPtr, "\0", 1) == 0)
            {
                qryPtr = questionPtr;
            }
            else
            {
                qryPtr = questionPtr + 1;
            }
            
            // Copy the query param
            memcpy(query, qryPtr, qryLength);
            

            // Get absolute-path (absPath)
            char absPath[questionPtr - (params[REQUEST_TARGET] + 1)];
            strncpy(absPath, params[REQUEST_TARGET], questionPtr - params[REQUEST_TARGET]);
            absPath[questionPtr - params[REQUEST_TARGET]] = '\0';
            
            // Ensure there is extension '.' else if not then error
            char* extDotPtr = strchr(absPath, '.');
            if (extDotPtr == NULL)
            {
                error(501);
                continue;
            ...
            // Extract extension (4 char max for this application + 1 char for end)
            char extension[endRT - extDotPtr];
            memcpy(extension, extDotPtr + 1, endRT - extDotPtr);
